...
class RegistrationFragment : Fragment() {
    ...
    private fun setupListener() = with(binding) {
        btnCode.setOnClickListener {
            ...
            if (binding?.edName?.text?.isNotEmpty() != null || binding?.edPass?.text?.isEmpty() != null) {
                PreferencesHelper.saveText = binding?.edName?.text.toString().trim()
                PreferencesHelper.saveText2 = binding?.edPass?.text.toString().trim()
